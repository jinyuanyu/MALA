...
def calculate_experiment_metrics(model, dataloader, out_channels, input_seq_len, device):
    ...
    for i, sample in enumerate(dataloader):
        ...
        original_masked_video = masked_video.clone()
        ...
        # 计算经典方法
        methods_results = calculate_classical_methods(original_masked_video, mask, device)
        methods_results["Proposed"] = combined
        
        # 计算指标
        metrics = calculate_all_metrics(video, original_masked_video, mask, methods_results, input_seq_len)
...
def calculate_classical_methods(masked_video, mask, device):
    """计算经典插值方法"""
    methods = {}
    input_copy = masked_video.clone()
    # DINEOF - 改进实现
    try:
        methods["DINEOF"] = apply_dineof_improved(input_copy, mask, device)
    except Exception as e:
        print(f"DINEOF计算失败: {e}")
        methods["DINEOF"] = input_copy.clone()
    
    # Nearest Neighbor
    try:
        methods["Nearest Neighbor"] = apply_nearest_neighbor_simple(input_copy, mask, device)
    except Exception as e:
        print(f"Nearest Neighbor计算失败: {e}")
        methods["Nearest Neighbor"] = input_copy.clone()
    
    # # Spline Interpolation
    try:
        methods["Spline"] = apply_spline_interpolation(input_copy, mask, device)
    except Exception as e:
        print(f"Spline计算失败: {e}")
        methods["Spline"] = input_copy.clone()
...
def save_sample_images(results, save_path):
    """保存 masked / mask / combined 三种对比图 - 修复版本"""
    def unnorm(img):
        return (img * 0.5 + 0.5).clamp(0, 1)

    root = f'experiment_results/{save_path}/images'
    os.makedirs(root, exist_ok=True)

    # --- 1. 原始输入（真实图像，用于对比） ---
    if 'video' in results:
        video = results['video']
        for t in range(min(8, video.shape[1])):
            img_t = unnorm(video[0, t])
            if img_t.shape[0] == 1:
                pic = Image.fromarray((img_t.squeeze(0).numpy() * 255).astype(np.uint8), 'L')
            else:
                pic = Image.fromarray((img_t.permute(1, 2, 0).numpy() * 255).astype(np.uint8), 'RGB')
            pic.save(os.path.join(root, f'original_frame{t:02d}.png'))

    # --- 2. 被掩膜的输入（缺失区域应该显示为黑色或特定值） ---
    if 'masked_video' in results and 'mask' in results:
        masked = results['masked_video']
        mask = results['mask']
        
        for t in range(min(8, masked.shape[1])):
            # 创建真正的masked版本：在缺失区域显示黑色
            masked_frame = unnorm(masked[0, t])  # 原始masked输入
            mask_frame = mask[0, t]  # 掩膜
            
            # 将缺失区域设为黑色以便可视化
            if mask_frame.shape[0] == 1:
                mask_3d = mask_frame.repeat(masked_frame.shape[0], 1, 1)
            else:
                mask_3d = mask_frame
            
            # 创建可视化版本：缺失区域为黑色，已知区域保持原值
            visual_masked = masked_frame * (1 - mask_3d)  # 缺失区域为黑色
            
            if visual_masked.shape[0] == 1:
                pic = Image.fromarray((visual_masked.squeeze(0).numpy() * 255).astype(np.uint8), 'L')
            else:
                pic = Image.fromarray((visual_masked.permute(1, 2, 0).numpy() * 255).astype(np.uint8), 'RGB')
            pic.save(os.path.join(root, f'masked_frame{t:02d}.png'))

    # --- 3. 掩膜本身 ---
    if 'mask' in results:
        mask_np = results['mask'][0].cpu().numpy()
        for t in range(min(8, mask_np.shape[0])):
            m = mask_np[t] * 255
            if m.ndim == 3 and m.shape[0] == 1:
                m = m.squeeze(0)
            Image.fromarray(m.astype(np.uint8), 'L').save(os.path.join(root, f'mask_frame{t:02d}.png'))

    # --- 4. 各方法的重建结果 ---
    if 'methods_results' in results:
        for name, result in results['methods_results'].items():
            method_dir = os.path.join(root, name.replace(' ', '_'))
            os.makedirs(method_dir, exist_ok=True)
            
            for t in range(min(8, result.shape[1])):
                img_t = unnorm(result[0, t])
                if img_t.shape[0] == 1:
                    pic = Image.fromarray((img_t.squeeze(0).numpy() * 255).astype(np.uint8), 'L')
                else:
                    pic = Image.fromarray((img_t.permute(1, 2, 0).numpy() * 255).astype(np.uint8), 'RGB')
                pic.save(os.path.join(method_dir, f'frame{t:02d}.png'))
                # 同时保存到根目录，保持向后兼容
                pic.save(os.path.join(root, f'{name}_combined_frame{t:02d}.png'))
