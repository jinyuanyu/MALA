...
def _resolve_data_path(path: Optional[str]) -> Optional[str]:
    """Resolve dataset paths across Linux/macOS/Windows-style E:/ layouts."""
    if not path:
        return path

    normalized = os.path.expanduser(path).replace("\\", "/")
    if os.path.exists(normalized):
        return normalized

    repo_relative = os.path.join(os.path.dirname(os.path.abspath(__file__)), normalized)
    if os.path.exists(repo_relative):
        return repo_relative

    mapped_root = os.environ.get("VMAE_DATA_ROOT")
    if mapped_root and normalized.startswith("E:/"):
        mapped_candidate = os.path.join(mapped_root, normalized[3:].lstrip("/"))
        if os.path.exists(mapped_candidate):
            return mapped_candidate

    return normalized

def _extract_frame_index(filename: str, fallback_idx: int) -> int:
    """Extract trailing numeric id from filename; fallback to sequence index."""
...
class Datasets(Dataset):
    ...
    def __init__(
        ...
        lama_init_dir: Optional[str] = None,
    ):
        self.data_dir = _resolve_data_path(data_dir)
        self.mask_dir = _resolve_data_path(mask_dir or os.environ.get("VMAE_MASK_DIR", "E:/lama/mask_img"))
        self.lama_init_dir = _resolve_data_path(
            lama_init_dir or os.environ.get("VMAE_LAMA_INIT_DIR", "E:/lama/inpainted_img/lama_init")
        )
        ...
        if self.img_list:
            ...
            raise ValueError("No images found in directory:", self.data_dir)
        #* 添加海洋掩码
        if ocean_mask_path is not None:
            ocean_mask = Image.open(_resolve_data_path(ocean_mask_path)).convert('L')
    ...
    def _load_data(self):
        img_list = []
        fname_list = []
        for idx, frame_file in enumerate(sorted(os.listdir(self.data_dir))):
            if frame_file.endswith('.png') or frame_file.endswith('.jpg'):
                frame_path = os.path.join(self.data_dir, frame_file)
                with Image.open(frame_path) as img:
                    image = img.copy()  # close file handle eagerly
                img_list.append(image)
                fname = _extract_frame_index(frame_file, idx)
    ...
    def _load_lama_init(self):
        lama_init = []
        if self.lama_init_dir and os.path.exists(self.lama_init_dir):
            for frame_file in sorted(os.listdir(self.lama_init_dir)):
                if frame_file.endswith('.png') or frame_file.endswith('.jpg'):
                    frame_path = os.path.join(self.lama_init_dir, frame_file)
    ...
    def __len__(self):
        return max(1, int(math.ceil(len(self.img_list) / self.max_seq_len)))
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_list is not None and len(self.mask_list) >= end_idx:
        ...
        if len(fnames) < self.max_seq_len:
            fnames = torch.cat([fnames, fnames[-1:].repeat(self.max_seq_len - len(fnames))], dim=0)
        ...
        if mask.shape[0] < self.max_seq_len:
            pad_mask = mask[-1:].repeat(self.max_seq_len - mask.shape[0], 1, 1, 1)
    ...
    def _generate_cloud_mask(self, mask_ratio: float):
        ...
        for t in range(self.max_seq_len):
            ...
            while current_masked < target_masked_pixels and attempts < max_attempts:
                # 生成云块大小：兼容小尺寸图像，避免 low >= high
                min_dim = max(8, int(min(self.img_size)))
                low = max(4, int(min_dim * 0.03))
                high = max(low + 1, int(min_dim * max(mask_ratio, 0.08)))
                cloud_size = np.random.randint(low, high + 1)
...
class LamaInpaintingModule(nn.Module):
    def __init__(self, device='cuda'):
        ...
        self.lama = SimpleLama() if SimpleLama is not None else None
...
def train(model, dataloader, optimizer, device, criterion, epochs=10, pretrained_path=None):
    ...
    try:
        ...
        # Compatibility for older torch versions without `verbose`.
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
